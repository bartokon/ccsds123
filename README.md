...
## HDL verification with cocotb

The repository ships a lightweight Python reference (`python_reference`) for the
local-difference stage. Run `make hdl-cocotb-local-diff` to regenerate the
stimulus vectors and exercise the HDL via cocotb. The make target invokes GHDL
twice (row- and column-oriented modes) and requires a working cocotb setup with
`cocotb-config` and `ghdl` on `PATH`.
